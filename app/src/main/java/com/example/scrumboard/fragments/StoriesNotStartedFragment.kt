...
class StoriesNotStartedFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_stories_not_started, container, false)
        ...
        val rv: RecyclerView = view.findViewById(R.id.rv_not_started)
        rv.layoutManager = LinearLayoutManager(requireContext())
        rv.hasFixedSize()
        rv.adapter = adapter
        viewModel.allTasks.observe(requireActivity(), {
           val notStartedTasksList : MutableList<Task> = mutableListOf()
           for (task in it){
               if (task.flag == 0){
                   notStartedTasksList.add(task)
               }
           }
            adapter.updateList(notStartedTasksList)
            rv.adapter = adapter
        })
        return view
    }
